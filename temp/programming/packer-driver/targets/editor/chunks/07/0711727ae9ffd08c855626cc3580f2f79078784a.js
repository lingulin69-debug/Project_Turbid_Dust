  ...
  return {
    ...
    execute: function () {
      ...
      _export("DonationTracker", DonationTracker = (_dec = ccclass('DonationTracker'), _dec2 = property(Label), _dec3 = property(ProgressBar), _dec4 = property(Node), _dec5 = property(Label), _dec6 = property(Label), _dec(_class = (_class2 = class DonationTracker extends Component {
        ...
        async initForChapter(chapterNumber) {
        ...
        async _loadDonationData() {
          try {
            const player = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
            }), DataManager) : DataManager).getPlayer();
            if (!player) return;
            const response = await fetch(`https://你的專案.supabase.co/rest/v1/td_player_donations?player_id=eq.${player.id}&chapter_number=eq.${this._currentChapter}`, {
              headers: {
        ...
        initTheme(faction) {
          ...
          if (this.progressBar) {
            ...
            const barSprite = (_this$progressBar$nod = this.progressBar.node.getChildByName('bar')) == null ? void 0 : _this$progressBar$nod.getComponent(Sprite);
        ...
        _updateDisplay() {
          ...
          const isMaxed = remaining <= 0; // 次數標籤
        ...
        async _onDonateClick() {
          ...
          tween(this.donateButton).to(0.05, {
        ...
        async _processDonation() {
          ...
          try {
            ...
            this._donationsThisWeek++; // 3. 寫入資料庫

            await fetch('https://你的專案.supabase.co/rest/v1/rpc/increment_donation', {
              method: 'POST',
              headers: {
                'apikey': 'YOUR_ANON_KEY',
                'Authorization': 'Bearer YOUR_ANON_KEY',
