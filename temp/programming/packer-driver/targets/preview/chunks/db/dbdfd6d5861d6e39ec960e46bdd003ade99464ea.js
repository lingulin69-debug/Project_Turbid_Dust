      ...
      _export("MapController", MapController = (_dec = ccclass('MapController'), _dec2 = property(Node), _dec3 = property(Prefab), _dec(_class = (_class2 = class MapController extends Component {
        ...
        _loadLandmarkData() {
          resources.load('data/landmark-chapters', JsonAsset, (err, asset) => {
            ...
            var chapters = (_chapters = asset.json.chapters) != null ? _chapters : [];
        ...
        _spawnLandmarks(dataList) {
          ...
          for (var data of dataList) {
            var node = instantiate(this.landmarkPrefab);
            node.setPosition(data.x, data.y, 0);
            this.mapRoot.addChild(node);
            var lm = node.getComponent(_crd && MapLandmark === void 0 ? (_reportPossibleCrUseOfMapLandmark({
              error: Error()
        ...
        getLandmarkWorldPos(id) {
          ...
          if (!lm) return null;
          var wp = lm.node.getWorldPosition();
          return {
            x: wp.x,
            y: wp.y
          };
        ...
        updateLandmark(data) {
          var lm = this._landmarks.get(data.id);
