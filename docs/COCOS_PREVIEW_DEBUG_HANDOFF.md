...
| 文件 | 存放內容 | 不放什麼 |
|------|---------|---------|
| **MANUAL**（手冊） | 操作指引、建節點步驟、Inspector 拖綁、設定教學、架構參考 | Bug 記錄、修復原因、偵錯分析 |
| **HANDOFF**（本文件） | Bug 記錄、修復根因、偵錯經驗、模式歸納、SOP | 操作步驟、建節點教學 |

> 新增內容時先問自己：「這是在教人怎麼做？」→ MANUAL。「這是在記錄出了什麼問題？」→ HANDOFF。
