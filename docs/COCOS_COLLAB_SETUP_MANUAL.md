...
> 外殼已經由程式自動建好了，你只需要在 BodyRoot（內容根節點）裡面確認幾個子節點存在就好。

### 60-1. Hierarchy（層級結構）

打開 Cocos Editor（編輯器），在左邊的 Node Tree（節點樹）裡找到：

```
PanelLayer（面板層）
  └── QuestPanelNode（任務面板節點）
        └── PanelBG（面板背景）
              └── BodyRoot（內容根節點）
                    ├── QuestContent（任務列表容器）
                    ├── QuestEmptyLabel（空列表提示文字）
                    └── QuestListScrollView（任務捲動視窗）← 可選
                          └── view（檢視框）
                                └── content（內容容器）
```

> 上面標示「可選」的節點，如果你不確定要不要建，可以先跳過，程式端會處理。

---

### 60-2. 新增 Empty Node（空節點）

如果 BodyRoot（內容根節點）下面還沒有 `QuestContent`，請手動新增：

1. 在 Node Tree（節點樹）中，**右鍵點擊** `BodyRoot`
2. 選 **Create（建立）** → **Empty Node（空節點）**
3. 把新節點改名為 `QuestContent`

接著設定它的 Properties（屬性）：
- 點選 `QuestContent` → 右邊會出現 Inspector（屬性檢查器）
- 找到 **UITransform** 元件 → 設定 **ContentSize（內容尺寸）**：W（寬）= `410`，H（高）= `320`
- 點 Inspector（屬性檢查器）最下方的 **Add Component（新增元件）** → 搜尋 `Layout（佈局）` → 加入
- Layout（佈局）設定：
  - Type（類型）= `VERTICAL`（垂直排列）
  - SpacingY（垂直間距）= `6`

---

### 60-3. 新增 UI Component（UI 元件）— Label（文字標籤）

在 BodyRoot（內容根節點）下新增一個文字提示：

1. **右鍵點擊** `BodyRoot` → **Create（建立）** → **2D Object（2D 物件）** → **Label（文字標籤）**
2. 把新節點改名為 `QuestEmptyLabel`

設定 Properties（屬性）：
- **UITransform** → ContentSize（內容尺寸）：W = `360`，H = `28`
- **Label（文字標籤）元件**：
  - String（文字內容）= `目前沒有任務`
  - FontSize（字體大小）= `14`
  - Horizontal Align（水平對齊）= `CENTER`（置中）
  - Color（顏色）= `(148, 163, 184, 255)` ← 淺灰色

---

### 60-4. 可選：新增 ScrollView（捲動視窗）

如果任務列表可能很長，你可以加一個 ScrollView（捲動視窗）：

1. **右鍵點擊** `BodyRoot` → **Create（建立）** → **2D Object（2D 物件）** → **ScrollView（捲動視窗）**
2. 改名為 `QuestListScrollView`
3. 它會自動產生 `view` > `content` 子節點

設定 Properties（屬性）：
- `QuestListScrollView` 的 **UITransform** → ContentSize：W = `410`，H = `320`
- **ScrollView 元件** → Direction（捲動方向）= `VERTICAL`（垂直）
- 展開 `view` → `content` → 點選 `content`：
  - **UITransform** → ContentSize：W = `410`，H = `320`
  - **Add Component（新增元件）** → `Layout（佈局）`
  - Layout 設定：Type = `VERTICAL`，SpacingY = `6`，ResizeMode（調整模式）= `CONTAINER`（跟隨容器）

---

### 60-5. Inspector（屬性檢查器）拖綁

在 Node Tree（節點樹）中，**點選** `QuestPanelNode` → 看右邊的 Inspector（屬性檢查器）→ 找到 **QuestPanel（腳本元件）**：

| 欄位名稱 | 要拖入的節點 | 說明 |
|---------|------------|------|
| bgSprite（背景圖片） | `PanelBG` | 把 PanelBG 從 Node Tree 拖進去 |
| titleLabel（標題文字） | `TitleLabel` | 把 TitleLabel 拖進去 |
| closeButton（關閉按鈕） | `CloseButton` | 把 CloseButton 拖進去 |
| contentContainer（內容容器） | `QuestContent` | 你剛剛建的那個空節點 |
| emptyLabel（空提示文字） | `QuestEmptyLabel` | 你剛剛建的 Label |
| scrollView（捲動視窗） | `QuestListScrollView` | 如果你有建的話拖進去，沒建就留空 |

---

### 60-6. 自我驗證 Checklist（檢查清單）

- [ ] `QuestPanelNode` 在 `PanelLayer` 下面，而且 Active（啟用）= 關閉 ❌
- [ ] `BodyRoot` 下面有 `QuestContent`（空節點 + Layout 垂直排列）
- [ ] `BodyRoot` 下面有 `QuestEmptyLabel`（Label 寫著「目前沒有任務」）
- [ ] Inspector（屬性檢查器）裡的 `contentContainer` 和 `emptyLabel` 已經拖好了
- [ ] 所有節點的 Layer（圖層）= `UI_2D`
- [ ] ❌ **不需要** 手動建任務列表裡面的每一筆 — 程式會動態產生
...
你需要在 PanelLayer（面板層）底下建出這樣的結構：

```
PanelLayer（面板層）
  └── PartyPanelNode（組隊面板節點）
        ├── Backdrop（背景遮罩）
        └── PanelBG（面板背景）
              ├── HeaderBar（頂部標題列）
              ├── TitleLabel（標題文字）
              ├── CloseButton（關閉按鈕）
              ├── CloseHintLabel（關閉提示文字）
              └── BodyRoot（內容根節點）
                    ├── PartyStatusLabel（隊伍狀態文字）
                    ├── MemberListScrollView（成員列表捲動視窗）
                    │     └── view（檢視框）
                    │           └── content（內容容器）
                    ├── ButtonBar（按鈕列）
                    │     ├── DrawCardBtn（抽牌按鈕）
...
**步驟 1：新增 PartyPanelNode（組隊面板節點）**

1. 在 Node Tree（節點樹）中，**右鍵點擊** `PanelLayer`
2. 選 **Create（建立）** → **Empty Node（空節點）**
3. 改名為 `PartyPanelNode`
4. Inspector（屬性檢查器）設定：
   - **UITransform** → ContentSize：W = `500`，H = `520`
   - **Add Component（新增元件）** → `Widget（對齊元件）` → HorizontalCenter（水平置中）= `0`，VerticalCenter（垂直置中）= `0`
5. **取消勾選 Active**（讓它一開始是隱藏的）
6. Layer（圖層）→ 選 `UI_2D` → 彈出的對話框選「Yes, change children」（套用到所有子節點）
...
2. Inspector 設定：
   - **UITransform** → ContentSize：W = `1280`，H = `720`（蓋滿整個畫面）
   - **Add Component** → `Sprite（精靈圖片）` → 不用指定圖片，只改 Color（顏色）= `(0, 0, 0, 150)` ← 半透明黑色
   - **Add Component** → `Button（按鈕）`（讓它可以被點擊）
   - ⚠️ **不要** 加 `BlockInputEvents`（阻擋輸入事件），因為要讓點擊穿透到底部關閉面板

---

### 61-4. 建立 PanelBG（面板背景）

這是面板的主要白色（深色）底板。

1. **右鍵點擊** `PartyPanelNode` → **Create** → **Empty Node** → 改名 `PanelBG`
2. Inspector 設定：
   - **UITransform** → ContentSize：W = `520`，H = `560`
   - **Add Component** → `Sprite` → Color = `(19, 24, 39, 245)` ← 深藍黑色
   - **Add Component** → `BlockInputEvents`（阻擋輸入事件）← ⚠️ **這裡要加**，防止點擊穿透到 Backdrop

---

### 61-5. 在 PanelBG（面板背景）下建立頂部元素

**HeaderBar（頂部標題列）：**
1. **右鍵** `PanelBG` → **Create** → **Empty Node** → 改名 `HeaderBar`
2. **UITransform** → ContentSize：W = `520`，H = `56`
3. **Position（位置）**：X = `0`，Y = `252`，Z = `0`
4. **Add Component** → `Sprite` → Color = `(74, 85, 104, 255)` ← 灰色

**TitleLabel（標題文字）：**
1. **右鍵** `PanelBG` → **Create** → **2D Object** → **Label** → 改名 `TitleLabel`
2. **UITransform** → ContentSize：W = `280`，H = `36`
3. **Position**：X = `0`，Y = `252`，Z = `0`
4. **Label 元件**：String = `組隊`，FontSize = `20`，HorizontalAlign = `CENTER`

**CloseButton（關閉按鈕）：**
1. **右鍵** `PanelBG` → **Create** → **2D Object** → **Label** → 改名 `CloseButton`
2. **UITransform** → ContentSize：W = `54`，H = `54`
3. **Position**：X = `218`，Y = `252`，Z = `0` ← 在右上角
4. **Label 元件**：String = `X`，FontSize = `34`，Color = `(248, 113, 113, 255)` ← 紅色
...
### 61-6. 建立 BodyRoot（內容根節點）

1. **右鍵** `PanelBG` → **Create** → **Empty Node** → 改名 `BodyRoot`
2. **UITransform** → ContentSize：W = `430`，H = `360`
3. **Position**：X = `0`，Y = `-20`，Z = `0`

---

### 61-7. 在 BodyRoot（內容根節點）下建立內容

**PartyStatusLabel（隊伍狀態文字）：**
1. **右鍵** `BodyRoot` → **Create** → **2D Object** → **Label** → 改名 `PartyStatusLabel`
2. **UITransform** → ContentSize：W = `380`，H = `30`
3. **Position**：X = `0`，Y = `150`，Z = `0`
4. **Label 元件**：String = `尚未加入隊伍`，FontSize = `15`，HorizontalAlign = `CENTER`，Color = `(248, 250, 252, 255)` ← 白色

**MemberListScrollView（成員列表捲動視窗）：**
1. **右鍵** `BodyRoot` → **Create** → **2D Object** → **ScrollView** → 改名 `MemberListScrollView`
2. **UITransform** → ContentSize：W = `380`，H = `180`
3. **Position**：X = `0`，Y = `30`，Z = `0`
4. **ScrollView 元件** → Direction = `VERTICAL`
5. 展開 `view` → 點選 `content`：
   - **UITransform** → ContentSize：W = `380`，H = `180`
   - **Add Component** → `Layout` → Type = `VERTICAL`，SpacingY = `6`，ResizeMode = `CONTAINER`

**ButtonBar（按鈕列）：**
1. **右鍵** `BodyRoot` → **Create** → **Empty Node** → 改名 `ButtonBar`
2. **UITransform** → ContentSize：W = `380`，H = `52`
3. **Position**：X = `0`，Y = `-80`，Z = `0`
4. **Add Component** → `Layout` → Type = `HORIZONTAL`（水平排列），SpacingX（水平間距）= `12`

在 ButtonBar 下建 3 個按鈕：

**DrawCardBtn（抽牌按鈕）：**
1. **右鍵** `ButtonBar` → **Create** → **Empty Node** → 改名 `DrawCardBtn`
2. **UITransform** → ContentSize：W = `110`，H = `42`
3. **Add Component** → `Sprite` → Color = `(168, 85, 247, 240)` ← 紫色
4. **Add Component** → `Button`
5. **右鍵** `DrawCardBtn` → **Create** → **2D Object** → **Label**
6. Label：String = `抽牌`，FontSize = `15`，HorizontalAlign = `CENTER`，Color = `(255, 255, 255, 255)` ← 白色
7. Label 的 **UITransform** → ContentSize：W = `110`，H = `42`

**JoinPartyBtn（加入隊伍按鈕）：**
1. **右鍵** `ButtonBar` → **Create** → **Empty Node** → 改名 `JoinPartyBtn`
2. **UITransform** → ContentSize：W = `110`，H = `42`
3. **Add Component** → `Sprite` → Color = `(34, 197, 94, 240)` ← 綠色
4. **Add Component** → `Button`
5. 裡面建一個 Label：String = `加入隊伍`，FontSize = `15`，HorizontalAlign = `CENTER`，白色
6. Label 的 UITransform：W = `110`，H = `42`

**EndPartyBtn（結束隊伍按鈕）：**
1. **右鍵** `ButtonBar` → **Create** → **Empty Node** → 改名 `EndPartyBtn`
2. **UITransform** → ContentSize：W = `110`，H = `42`
3. **Add Component** → `Sprite` → Color = `(239, 68, 68, 240)` ← 紅色
4. **Add Component** → `Button`
5. 裡面建一個 Label：String = `結束隊伍`，FontSize = `15`，HorizontalAlign = `CENTER`，白色
6. Label 的 UITransform：W = `110`，H = `42`
7. ⚠️ **取消勾選 Active** ← 預設隱藏，只有隊長才會顯示

**PartyStoryScrollView（組隊敘事捲動視窗）：**
1. **右鍵** `BodyRoot` → **Create** → **2D Object** → **ScrollView** → 改名 `PartyStoryScrollView`
2. **UITransform** → ContentSize：W = `380`，H = `120`
3. **Position**：X = `0`，Y = `-170`，Z = `0`
4. **ScrollView 元件** → Direction = `VERTICAL`
5. 展開 `view` → `content` → 在 `content` 下建一個 Label：
   - 改名 `StoryLabel`
   - **UITransform** → ContentSize：W = `360`，H = `100`
   - **Label 元件**：String = `組隊敘事將顯示在此`，FontSize = `12`，Color = `(203, 213, 225, 255)` ← 淺灰色
...
### 61-10. 自我驗證 Checklist（檢查清單）

- [ ] `PartyPanelNode` 在 `PanelLayer` 下面，Active = 關閉
- [ ] `Backdrop`：尺寸 1280×720，半透明黑色，有 Button，**沒有** BlockInputEvents
- [ ] `PanelBG`：尺寸 520×560，深色底，**有** BlockInputEvents
...
- [ ] `MemberListScrollView` > `view` > `content`：有 Layout 垂直排列
- [ ] `ButtonBar`：Layout 水平排列，尺寸 380×52，位置 (0, -80, 0)
- [ ] `DrawCardBtn`：紫色，文字 =「抽牌」，Active = 開啟
- [ ] `JoinPartyBtn`：綠色，文字 =「加入隊伍」，Active = 開啟
...
## 62. NPCModalNode（NPC 商人交易面板）

> 外殼已經由程式自動建好了。這一節是在已有的 NPCModalNode 裡面加入商店 Tab（分頁）功能。
...
```
NPCModalNode（NPC 面板節點）← 已存在
  └── PanelBG（面板背景）← 已存在
        ├── （已有的 HeaderBar、TitleLabel 等）
        ├── TabBar（分頁標籤列）← 🎮 新建
        │     ├── TabBtn_Dialog（對話分頁按鈕）
        │     └── TabBtn_Shop（商店分頁按鈕）
        └── BodyRoot（內容根節點）← 已存在
              ├── （已有的 DialogueLabel、ShopContainer 等）
              └── ShopTabContainer（商店分頁容器）← 🎮 新建
                    ├── ShopScrollView（商品捲動視窗）
                    │     └── view（檢視框）
                    │           └── content（內容容器）
                    └── BuyConfirmBtn（確認購買按鈕）
```

---

### 62-2. 在 PanelBG（面板背景）下新增 TabBar（分頁標籤列）

⚠️ **注意：TabBar 是建在 PanelBG 底下，不是 BodyRoot 底下！**

1. **右鍵** `PanelBG` → **Create** → **Empty Node** → 改名 `TabBar`
2. **UITransform** → ContentSize：W = `300`，H = `36`
3. **Position**：X = `0`，Y = `220`，Z = `0` ← 在標題列下方
4. **Add Component** → `Layout` → Type = `HORIZONTAL`，SpacingX = `8`

**TabBtn_Dialog（對話分頁按鈕）：**
1. **右鍵** `TabBar` → **Create** → **Empty Node** → 改名 `TabBtn_Dialog`
2. **UITransform** → ContentSize：W = `90`，H = `30`
3. **Add Component** → `Sprite` → Color = `(59, 130, 246, 240)` ← 藍色（表示目前選中）
4. **Add Component** → `Button`
5. 裡面建一個 Label：String = `對話`，FontSize = `14`，HorizontalAlign = `CENTER`，白色
6. Label 的 UITransform：W = `90`，H = `30`

**TabBtn_Shop（商店分頁按鈕）：**
1. **右鍵** `TabBar` → **Create** → **Empty Node** → 改名 `TabBtn_Shop`
2. **UITransform** → ContentSize：W = `90`，H = `30`
3. **Add Component** → `Sprite` → Color = `(71, 85, 105, 240)` ← 灰色（表示未選中）
4. **Add Component** → `Button`
5. 裡面建一個 Label：String = `商店`，FontSize = `14`，HorizontalAlign = `CENTER`，白色
6. Label 的 UITransform：W = `90`，H = `30`

---

### 62-3. 在 BodyRoot（內容根節點）下新增 ShopTabContainer（商店分頁容器）

1. **右鍵** `BodyRoot` → **Create** → **Empty Node** → 改名 `ShopTabContainer`
2. **UITransform** → ContentSize：W = `430`，H = `340`
3. ⚠️ **取消勾選 Active** ← 預設隱藏，切到商店 Tab 時才顯示

**ShopScrollView（商品捲動視窗）：**
1. **右鍵** `ShopTabContainer` → **Create** → **2D Object** → **ScrollView** → 改名 `ShopScrollView`
2. **UITransform** → ContentSize：W = `410`，H = `270`
3. **Position**：X = `0`，Y = `15`，Z = `0`
4. **ScrollView 元件** → Direction = `VERTICAL`
5. 展開 `view` → 點選 `content`：
   - **UITransform** → ContentSize：W = `410`，H = `270`
   - **Add Component** → `Layout` → Type = `VERTICAL`，SpacingY = `6`，ResizeMode = `CONTAINER`

**BuyConfirmBtn（確認購買按鈕）：**
1. **右鍵** `ShopTabContainer` → **Create** → **Empty Node** → 改名 `BuyConfirmBtn`
2. **UITransform** → ContentSize：W = `120`，H = `40`
3. **Position**：X = `0`，Y = `-145`，Z = `0`
4. **Add Component** → `Sprite` → Color = `(59, 130, 246, 240)` ← 藍色
5. **Add Component** → `Button`
6. 裡面建一個 Label：String = `確認購買`，FontSize = `14`，HorizontalAlign = `CENTER`，白色
7. Label 的 UITransform：W = `120`，H = `40`

---

### 62-4. Inspector（屬性檢查器）拖綁

等程式端在 NPCModal（NPC 面板）腳本加好對應欄位後：

點選 `NPCModalNode` → Inspector → 找到 **NPCModal 腳本元件**：

| 欄位名稱 | 要拖入的節點 | 說明 |
|---------|------------|------|
| shopTabContainer（商店分頁容器） | `ShopTabContainer` | 你剛剛建的那個在 BodyRoot 下的空節點 |
| tabBtnDialog（對話分頁按鈕） | `TabBtn_Dialog` | 在 TabBar 下的藍色按鈕 |
| tabBtnShop（商店分頁按鈕） | `TabBtn_Shop` | 在 TabBar 下的灰色按鈕 |
| shopScrollContent（商品捲動內容） | `content`（ShopScrollView > view > content） | 拖 ScrollView 裡面最深層的 content 節點 |

---

### 62-5. 自我驗證 Checklist（檢查清單）

- [ ] `TabBar` 在 **PanelBG** 下面（不是 BodyRoot），尺寸 300×36，位置 (0, 220, 0)
- [ ] `TabBar` 有 Layout 水平排列，SpacingX = 8
- [ ] `TabBtn_Dialog`：藍色 (59,130,246)，文字 =「對話」
- [ ] `TabBtn_Shop`：灰色 (71,85,105)，文字 =「商店」
- [ ] `ShopTabContainer` 在 **BodyRoot** 下面，尺寸 430×340，**Active = 關閉**
- [ ] `ShopScrollView`：ScrollView 垂直捲動，尺寸 410×270，位置 (0, 15, 0)
- [ ] `ShopScrollView` > `view` > `content`：有 Layout 垂直排列，ResizeMode = CONTAINER
- [ ] `BuyConfirmBtn`：藍色，尺寸 120×40，位置 (0, -145, 0)，文字 =「確認購買」
- [ ] 所有節點 Layer = `UI_2D`
- [ ] ❌ 商品列表不用手動建 — 程式會動態產生
...
```
PanelLayer（面板層）
  └── DriftFragmentPanelNode（漂流瓶面板節點）
        ├── Backdrop（背景遮罩）
        └── PanelBG（面板背景）
              ├── HeaderBar（頂部標題列）
              ├── TitleLabel（標題文字）
              ...
              ├── CloseHintLabel（關閉提示文字）
              └── BodyRoot（內容根節點）
                    ├── FragmentScrollView（漂流瓶列表捲動視窗）
                    │     └── view（檢視框）
                    │           └── content（內容容器）
                    ├── WriteArea（撰寫區域）
                    │     ├── ContentEditBox（內容輸入框）
                    │     └── SendBtn（投放按鈕）
                    └── EmptyLabel（空列表提示文字）
```
...
2. **UITransform** → ContentSize：W = `420`，H = `460`
3. **Add Component** → `Widget` → HorizontalCenter = `0`，VerticalCenter = `0`
4. **取消勾選 Active**
5. Layer → `UI_2D`（套用到子節點）
...
- Sprite Color = `(0, 0, 0, 150)` + Button，**不要** 加 BlockInputEvents

**PanelBG：**
- 父節點：`DriftFragmentPanelNode`
- UITransform：520 × 560
- Sprite Color = `(19, 24, 39, 245)` + **加** BlockInputEvents

---

### 63-4. 在 PanelBG（面板背景）下建立頂部元素

做法跟 §61-5 完全一樣，只有標題文字不同：

| 節點 | 跟 §61 的差異 |
|------|-------------|
| HeaderBar（頂部標題列） | 完全一樣 |
| TitleLabel（標題文字） | String = `漂流瓶` ← 只改這裡 |
| CloseButton（關閉按鈕） | 完全一樣 |
| CloseHintLabel（關閉提示文字） | 完全一樣 |

---

### 63-5. 建立 BodyRoot（內容根節點）
...
### 63-6. 在 BodyRoot（內容根節點）下建立內容

**FragmentScrollView（漂流瓶列表捲動視窗）：**
1. **右鍵** `BodyRoot` → **Create** → **2D Object** → **ScrollView** → 改名 `FragmentScrollView`
2. **UITransform** → ContentSize：W = `380`，H = `220`
3. **Position**：X = `0`，Y = `30`，Z = `0`
4. **ScrollView 元件** → Direction = `VERTICAL`
5. 展開 `view` → 點選 `content`：
...
**WriteArea（撰寫區域）：**
1. **右鍵** `BodyRoot` → **Create** → **Empty Node** → 改名 `WriteArea`
2. **UITransform** → ContentSize：W = `380`，H = `100`
3. **Position**：X = `0`，Y = `-120`，Z = `0`

在 WriteArea 裡面建兩個東西：

**ContentEditBox（內容輸入框）：**
1. **右鍵** `WriteArea` → **Create** → **2D Object** → **EditBox（輸入框）** → 改名 `ContentEditBox`
2. **UITransform** → ContentSize：W = `280`，H = `70`
3. **Position**：X = `-40`，Y = `0`，Z = `0`
4. **EditBox 元件** 設定：
   - InputMode（輸入模式）= `ANY`（任意輸入）
   - MaxLength（最大長度）= `100`
   - Placeholder（佔位文字）= `寫下你的漂流瓶訊息...`
   - FontSize = `13`

**SendBtn（投放按鈕）：**
1. **右鍵** `WriteArea` → **Create** → **Empty Node** → 改名 `SendBtn`
2. **UITransform** → ContentSize：W = `80`，H = `36`
3. **Position**：X = `155`，Y = `0`，Z = `0` ← 在輸入框右邊
4. **Add Component** → `Sprite` → Color = `(59, 130, 246, 240)` ← 藍色
5. **Add Component** → `Button`
6. 裡面建一個 Label：String = `投放`，FontSize = `14`，HorizontalAlign = `CENTER`，白色
7. Label 的 UITransform：W = `80`，H = `36`

**EmptyLabel（空列表提示文字）：**
1. **右鍵** `BodyRoot` → **Create** → **2D Object** → **Label** → 改名 `EmptyLabel`
2. **UITransform** → ContentSize：W = `360`，H = `28`
3. **Position**：X = `0`，Y = `30`，Z = `0` ← 跟 ScrollView 同位置，程式會控制顯示/隱藏
4. **Label 元件**：String = `目前沒有漂流瓶`，FontSize = `14`，HorizontalAlign = `CENTER`，Color = `(148, 163, 184, 255)` ← 淺灰
...
### 63-8. 自我驗證 Checklist（檢查清單）

- [ ] `DriftFragmentPanelNode` 在 `PanelLayer` 下面，Active = 關閉
- [ ] `Backdrop`：尺寸 1280×720，半透明黑色，有 Button，**沒有** BlockInputEvents
- [ ] `PanelBG`：尺寸 520×560，深色底，**有** BlockInputEvents
- [ ] `HeaderBar`：尺寸 520×56，位置 (0, 252, 0)
- [ ] `TitleLabel`：文字 =「漂流瓶」，位置 (0, 252, 0)
- [ ] `CloseButton`：尺寸 54×54，位置 (218, 252, 0)，紅色 X
- [ ] `CloseHintLabel`：文字 =「點空白區域也可關閉」
- [ ] `BodyRoot`：尺寸 430×360，位置 (0, -20, 0)
- [ ] `FragmentScrollView`：ScrollView 垂直捲動，尺寸 380×220，位置 (0, 30, 0)
- [ ] `FragmentScrollView` > `view` > `content`：有 Layout 垂直排列
- [ ] `WriteArea`：尺寸 380×100，位置 (0, -120, 0)
  ...
  └── GazettePanelNode（公報面板節點）
        ...
        └── PanelBG（面板背景）
              ...
              └── BodyRoot（內容根節點）
                    ├── FilterBar（篩選列）
                    │     ├── FilterBtn_All（全部按鈕）
                    │     ├── FilterBtn_Mission（任務按鈕）
                    │     ├── FilterBtn_System（系統按鈕）
                    │     └── FilterBtn_Leader（領袖按鈕）
                    ├── GazetteScrollView（公報捲動視窗）
                    │     └── view（檢視框）
                    │           └── content（內容容器）
                    └── GazetteEmptyLabel（空列表提示文字）
```
...
2. **UITransform** → ContentSize：W = `480`，H = `520`
3. **Add Component** → `Widget` → HorizontalCenter = `0`，VerticalCenter = `0`
4. **取消勾選 Active**
5. Layer → `UI_2D`（套用到子節點）
...
|------|-------------|
| HeaderBar（頂部標題列） | 完全一樣 |
| TitleLabel（標題文字） | String = `公報` ← 只改這裡 |
| CloseButton（關閉按鈕） | 完全一樣 |
| CloseHintLabel（關閉提示文字） | 完全一樣 |

---

### 64-5. 建立 BodyRoot（內容根節點）
...
### 64-6. 在 BodyRoot（內容根節點）下建立 FilterBar（篩選列）

1. **右鍵** `BodyRoot` → **Create** → **Empty Node** → 改名 `FilterBar`
2. **UITransform** → ContentSize：W = `400`，H = `32`
3. **Position**：X = `0`，Y = `150`，Z = `0` ← 在頂部
4. **Add Component** → `Layout` → Type = `HORIZONTAL`，SpacingX = `6`

在 FilterBar 下面建 4 個篩選按鈕（做法重複，只有文字和顏色不同）：

**FilterBtn_All（全部按鈕）：**
1. **右鍵** `FilterBar` → **Create** → **Empty Node** → 改名 `FilterBtn_All`
2. **UITransform** → ContentSize：W = `60`，H = `28`
3. **Add Component** → `Sprite` → Color = `(59, 130, 246, 240)` ← 藍色（預設選中）
4. **Add Component** → `Button`
5. 裡面建一個 Label：String = `全部`，FontSize = `12`，HorizontalAlign = `CENTER`，白色
6. Label 的 UITransform：W = `60`，H = `28`

**FilterBtn_Mission（任務按鈕）：**
1. 做法跟上面一樣，改名 `FilterBtn_Mission`
2. Sprite Color = `(71, 85, 105, 240)` ← 灰色（未選中）
3. Label String = `任務`

**FilterBtn_System（系統按鈕）：**
1. 做法一樣，改名 `FilterBtn_System`
2. Sprite Color = `(71, 85, 105, 240)` ← 灰色
3. Label String = `系統`

**FilterBtn_Leader（領袖按鈕）：**
1. 做法一樣，改名 `FilterBtn_Leader`
2. Sprite Color = `(71, 85, 105, 240)` ← 灰色
3. Label String = `領袖`

---

### 64-7. 在 BodyRoot 下建立 GazetteScrollView（公報捲動視窗）

1. **右鍵** `BodyRoot` → **Create** → **2D Object** → **ScrollView** → 改名 `GazetteScrollView`
2. **UITransform** → ContentSize：W = `420`，H = `320`
3. **Position**：X = `0`，Y = `-25`，Z = `0`
4. **ScrollView 元件** → Direction = `VERTICAL`
5. 展開 `view` → 點選 `content`：
...
### 64-8. 在 BodyRoot 下建立 GazetteEmptyLabel（空列表提示文字）

1. **右鍵** `BodyRoot` → **Create** → **2D Object** → **Label** → 改名 `GazetteEmptyLabel`
2. **UITransform** → ContentSize：W = `360`，H = `28`
3. **Label 元件**：String = `目前沒有公報`，FontSize = `14`，HorizontalAlign = `CENTER`，Color = `(148, 163, 184, 255)` ← 淺灰
...
### 64-10. 自我驗證 Checklist（檢查清單）

- [ ] `GazettePanelNode` 在 `PanelLayer` 下面，Active = 關閉
- [ ] `Backdrop`：尺寸 1280×720，半透明黑色，有 Button，**沒有** BlockInputEvents
- [ ] `PanelBG`：尺寸 520×560，深色底，**有** BlockInputEvents
- [ ] `HeaderBar`：尺寸 520×56，位置 (0, 252, 0)
- [ ] `TitleLabel`：文字 =「公報」
- [ ] `CloseButton`：紅色 X，位置 (218, 252, 0)
- [ ] `BodyRoot`：尺寸 430×360，位置 (0, -20, 0)
- [ ] `FilterBar`：Layout 水平排列，尺寸 400×32，位置 (0, 150, 0)
- [ ] `FilterBtn_All`：藍色，文字 =「全部」
- [ ] `FilterBtn_Mission`：灰色，文字 =「任務」
- [ ] `FilterBtn_System`：灰色，文字 =「系統」
- [ ] `FilterBtn_Leader`：灰色，文字 =「領袖」
...
## 65. OverlayLayer（覆蓋層）視覺效果節點

> 這一節很簡單。等美術素材到手後才會真正用到，現在只是先建好空節點佔位。
...
OverlayLayer（覆蓋層）
  ├── FogLayerNode（迷霧層節點）
  └── GlitchGhostNode（故障幽靈節點）
```

---

### 65-2. 建立 FogLayerNode（迷霧層節點）

1. 在 Node Tree 找到 `OverlayLayer` → **右鍵** → **Create** → **Empty Node** → 改名 `FogLayerNode`
2. **UITransform** → ContentSize：W = `1280`，H = `720`（蓋滿整個畫面）
3. **Add Component** → `Widget` → 把 Top / Bottom / Left / Right 四邊都勾選，值都設 `0`（全拉伸）
4. **取消勾選 Active** ← 預設隱藏
5. ⚠️ **不要** 加 Sprite — 等美術素材到手再加

---

### 65-3. 建立 GlitchGhostNode（故障幽靈節點）

1. **右鍵** `OverlayLayer` → **Create** → **Empty Node** → 改名 `GlitchGhostNode`
2. **UITransform** → ContentSize：W = `200`，H = `200`
3. **取消勾選 Active** ← 預設隱藏

---

### 65-4. 確認 Material（材質）接口

隨便點一個面板節點（例如 `QuestPanelNode`）→ Inspector → 找到面板腳本 → 應該會看到：

- `Turbid Material（濁息材質）`：空欄位（類型 = Material）
- `Pure Material（淨塵材質）`：空欄位（類型 = Material）

現在空著是正確的。等美術給 `.mtl` 檔案後，直接拖進去就好。

---

### 65-5. 不需操作的部分

面板的動畫效果（淡入淡出、縮放彈出）由程式用 `tween()` 處理，不需要你在 Editor 裡設定任何 Animation 元件。

---

### 65-6. 自我驗證 Checklist（檢查清單）

- [ ] `FogLayerNode` 在 `OverlayLayer` 下面，Active = 關閉
- [ ] `FogLayerNode` 的 UITransform = 1280×720，Widget 四邊拉伸
- [ ] `FogLayerNode` **沒有** Sprite（空節點就好）
- [ ] `GlitchGhostNode` 在 `OverlayLayer` 下面，Active = 關閉
- [ ] `GlitchGhostNode` 的 UITransform = 200×200
- [ ] 隨便一個面板都有 Turbid Material / Pure Material 空欄位
- [ ] 兩個節點 Layer = `UI_2D`

---

## 66. Phase 6-8（開發階段 6-8）整體驗證清單

> 這是最後的大驗證。Server（伺服器）要先跑著：在終端機輸入 `npm run server`

### Phase 6（階段 6）驗證 — API（接口）接通

1. Preview（預覽）→ 打開 InventoryPanel（背包面板）→ 物品不再是 8 筆固定假資料
2. Preview → 打開 ApostatePanel（叛教者面板）→ 送出問卷 → Console（控制台）看到：`submitScreening 成功`
3. Preview → 打開 LiquidatorPanel（清算者面板）→ 掃描 → Console 看到：`liquidatorScan`
4. Preview → 打開 LeaderTyrannyPanel（暴政面板）→ 點「徵稅」→ Console 看到：`leader.tax 成功`
5. Preview → 打開 BalanceSettlementModal（天平結算面板）→ Console 看到：`getSettlementResult`
6. Preview → 打開 LeaderboardPanel（排行榜面板）→ Console 看到：`fetchLeaderboard`

### Phase 7（階段 7）驗證 — 新系統面板

1. QuestPanel（任務面板）→ 任務列表有回報按鈕
2. PartyPanel（組隊面板）→ 狀態文字 + 3 個按鈕 + 敘事區
3. NPCModal（NPC 面板）→ 點「商店」Tab → 商品列表出現
4. DriftFragmentPanel（漂流瓶面板）→ 漂流瓶列表 + 撰寫區
5. GazettePanel（公報面板）→ FilterBar + 公報列表出現

### Phase 8（階段 8）驗證 — 視覺效果

1. OverlayLayer 下有 FogLayerNode 和 GlitchGhostNode（Active = 關閉）
2. 面板 Inspector 有 Turbid Material / Pure Material 空欄位
3. 拖入 Material → Preview → 面板外觀有變化
